...
class EventBuyerController extends Controller {
  ...
  public function index(Request $request) {
    try {
      return $this->rsp(200, 'Registros retornados correctamente', [
        'items' => EventBuyer::getItems($request),
      ]);
    } catch (Throwable $err) {
  ...
  public function show(string $id, Request $request) {
    try {
      $item = EventBuyer::getItem($id, $request);
  ...
  public function destroy(string $id, Request $request) {
    return $this->setActive(EventBuyer::class, $id, $request, false);
  }

  public function activate(string $id, Request $request) {
    return $this->setActive(EventBuyer::class, $id, $request, true);
  }
  ...
  protected function storeUpdate(?string $id, Request $request) {
    ...
    try {
      ...
      $valid = EventBuyer::validData($request->all());
      if ($valid->fails()) {
        DB::rollBack();
      ...
      $item = EventBuyer::saveData($item, $payload);
