...
class TicketTypeController extends Controller {
  ...
  public function index(Request $request) {
    try {
      return $this->rsp(200, 'Registros retornados correctamente', [
        'items' => TicketType::getItems($request),
      ]);
    } catch (Throwable $err) {
  ...
  public function show(string $id, Request $request) {
    try {
      $item = TicketType::getItem($id, $request);
  ...
  public function destroy(string $id, Request $request) {
    return $this->setActive(TicketType::class, $id, $request, false);
  }

  public function activate(string $id, Request $request) {
    return $this->setActive(TicketType::class, $id, $request, true);
  }
  ...
  protected function storeUpdate(?string $id, Request $request) {
    ...
    try {
      ...
      $valid = TicketType::validData($request->all());
      if ($valid->fails()) {
        DB::rollBack();
      ...
      $item = TicketType::saveData($item, $payload);
