...
class EventStandConfigController extends Controller {
  use HasActiveToggle;
  ...
  public function index(Request $request) {
    try {
      return $this->rsp(200, 'Registros retornados correctamente', [
        'items' => EventStandConfig::getItems($request),
      ]);
    } catch (Throwable $err) {
  ...
  public function show(string $id, Request $request) {
    try {
      $item = EventStandConfig::getItem($id, $request);
  ...
  public function destroy(string $id, Request $request) {
    return $this->setActive(EventStandConfig::class, $id, $request, false);
  }

  public function activate(string $id, Request $request) {
    return $this->setActive(EventStandConfig::class, $id, $request, true);
  }
  ...
  protected function storeUpdate(?string $id, Request $request) {
    ...
    try {
      ...
      $valid = EventStandConfig::validData($request->all());
      if ($valid->fails()) {
        DB::rollBack();
      ...
      $item = EventStandConfig::saveData($item, $payload);
