...
class EventMeetingWindowController extends Controller
{
  ...
  public function index(Request $request) {
    try {
      return $this->rsp(200, 'Registros retornados correctamente', [
        'items' => EventMeetingWindow::getItems($request),
      ]);
    } catch (Throwable $err) {
  ...
  public function show(string $id, Request $request) {
    try {
      $item = EventMeetingWindow::getItem($id, $request);
  ...
  public function destroy(string $id, Request $request) {
    return $this->setActive(EventMeetingWindow::class, $id, $request, false);
  }

  public function activate(string $id, Request $request) {
    return $this->setActive(EventMeetingWindow::class, $id, $request, true);
  }
  ...
  protected function storeUpdate(?string $id, Request $request) {
    ...
    try {
      ...
      $valid = EventMeetingWindow::validData($request->all());
      if ($valid->fails()) {
        DB::rollBack();
      ...
      if ($store_mode) {
        $item = new EventMeetingWindow();
      } else {
        $item = EventMeetingWindow::find((int) $id);
      ...
      $item = EventMeetingWindow::saveData($item, $payload);
