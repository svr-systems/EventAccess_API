...
class PresentationDateController extends Controller {
  ...
  public function index(Request $request) {
    try {
      return $this->rsp(200, 'Registros retornados correctamente', [
        'items' => PresentationDate::getItems($request),
      ]);
    } catch (Throwable $err) {
  ...
  public function show(string $id, Request $request) {
    try {
      $item = PresentationDate::getItem($id, $request);
  ...
  public function destroy(string $id, Request $request) {
    return $this->setActive(PresentationDate::class, $id, $request, false);
  }

  public function activate(string $id, Request $request) {
    return $this->setActive(PresentationDate::class, $id, $request, true);
  }
  ...
  protected function storeUpdate(?string $id, Request $request) {
    ...
    try {
      ...
      $valid = PresentationDate::validData($request->all());
      if ($valid->fails()) {
        DB::rollBack();
      ...
      $item = PresentationDate::saveData($item, $payload);
  ...
  public function publicIndex(Request $request) {
    try {
      return $this->rsp(200, 'Registros retornados correctamente', [
        'items' => PresentationDate::publicGetItems($request),
      ]);
    } catch (Throwable $err) {
      return $this->rsp(500, null, $err);
    }
  ...
  public function buyersIndex(Request $request) {
    try {
      return $this->rsp(200, 'Registros retornados correctamente', [
        'items' => PresentationDate::getBuyersItems($request),
      ]);
    } catch (Throwable $err) {
      return $this->rsp(500, null, $err);
    }
