...
class EventSupplierController extends Controller
{
  ...
  public function index(Request $request) {
    try {
      return $this->rsp(200, 'Registros retornados correctamente', [
        'items' => EventSupplier::getItems($request),
      ]);
    } catch (Throwable $err) {
  ...
  public function show(string $id, Request $request) {
    try {
      $item = EventSupplier::getItem($id, $request);
  ...
  public function destroy(string $id, Request $request) {
    return $this->setActive(EventSupplier::class, $id, $request, false);
  }

  public function activate(string $id, Request $request) {
    return $this->setActive(EventSupplier::class, $id, $request, true);
  }
  ...
  protected function storeUpdate(?string $id, Request $request) {
    ...
    try {
      ...
      $valid = EventSupplier::validData($request->all());
      if ($valid->fails()) {
        DB::rollBack();
      ...
      if ($store_mode) {
        $item = new EventSupplier();
      } else {
        $item = EventSupplier::find((int) $id);
      ...
      $item = EventSupplier::saveData($item, $payload);
